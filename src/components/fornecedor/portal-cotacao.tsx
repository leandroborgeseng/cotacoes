...
function requisitosLegivel(jsonOuTexto: string | undefined): string {
  if (!jsonOuTexto?.trim()) return "—";
  try {
    const a = JSON.parse(jsonOuTexto) as unknown;
    if (Array.isArray(a)) return a.map((x) => String(x)).join(" · ");
  } catch {
    /* texto livre */
  }
  return jsonOuTexto.trim();
}
                  ...
                  <Table className="min-w-[880px]">
                    <TableHeader>
                      <TableRow className="border-border/60 bg-muted/50 hover:bg-muted/50">
                        ...
                        <TableHead className="w-16 text-right bg-muted/50 px-2">Qtd</TableHead>
                    ...
                    <TableBody>
                      {q.data.equipamentos.map((eq) => (
                        <TableRow key={eq.id} className="border-border/40">
                          ...
                          <TableCell className="bg-card px-2 text-right tabular-nums font-medium">{eq.quantidade}</TableCell>
